...
# Converts a number to its two's compliment binary (saturates when values are larger then number of bits)
...
if testing_overflow_underflow:
    input_file = open("tb_input_pt3_flow.txt", "w")
    output_file = open("tb_output_pt3_flow.txt", "w")
    io_test_file = open("tb_io_pt3_flow.txt", "w")
else:
    input_file = open("tb_input_pt3.txt", "w")
    output_file = open("tb_output_pt3.txt", "w")
    io_test_file = open("tb_io_pt3.txt", "w")

# Generates a new random input or use an already defined input text file
if random_generate == True:
    if testing_overflow_underflow:
        txt_input = open("input_pt3_flow.txt", "w")
    else:
        txt_input = open("input_pt3.txt", "w")
else:
    if testing_overflow_underflow:
        txt_input = open("input_pt3_flow.txt", "r")
    else:
        txt_input = open("input_pt3.txt", "r")
...
for i in range(number_of_test_cases):
    ...
    # get the delayed signals and outputs
